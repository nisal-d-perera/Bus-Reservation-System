...
if ($_GET['p_id']) {
    $sql = "SELECT * FROM `payment` WHERE `payment_id`='".$_GET['p_id']."';";
    $result = $conn->query($sql);
    $row = $result->fetch_assoc();
                        ...
                        <div class="col-lg-7">
                            <div class="p-5">
                                <div class="text-center">
                                    <h1 class="h4 text-gray-900 mb-4"> Payment ID [<?=$_GET['p_id']?>]</h1>
                                </div>
                                <form class="user" method="post">
                                    <h4>Do you really want to delete both ticket and payment details?</h4><br>
                                    <h5>Ticket ID : <?=$tc_id?></h5><br>
                                    <h5>Payment Amount : <?=$p_amount?></h5><br>
                                    <h5>Payment Date : <?=$p_date?></h5><br>
                                    <br>
                                    <center>
...
if (isset($_POST['delete'])) {
    ...
    $sql = "SELECT `tic_status` FROM `ticket` WHERE  `ticket`.`ticket_id`='$tc_id';";
    $result = $conn->query($sql);
    $row = $result->fetch_assoc();
    $tc_s = $row["tic_status"];
    ...
    if($tc_s == "Confirm"){
        ...
        echo "<script>alert('Cannot delete details. Ticket is not expired');document.location='Ticket.php'</script>";

    }
    else{

        $sql="DELETE FROM `ticket` WHERE ticket.ticket_id = '$tc_id'";
        $conn->query($sql);

        $sql="DELETE FROM `payment` WHERE payment.payment_id = '$p_id'";
        $conn->query($sql);
